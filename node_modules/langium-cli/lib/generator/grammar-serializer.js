...
function serializeGrammar(services, grammars, config) {
    ...
    if (config.langiumInternal) {
        node.append("import { loadGrammarFromJson } from '../../utils/grammar-util';", langium_1.NL, "import { Grammar } from './ast';");
    }
    else {
    ...
    for (let i = 0; i < grammars.length; i++) {
        ...
        if (grammar.name) {
            ...
            const json = (0, langium_1.normalizeEOL)(services.serializer.JsonSerializer.serialize(grammar, { space: 2 }).replace(/\\/g, '\\\\').replace(/`/g, '\\`'));
            node.append('let loaded', grammar.name, 'Grammar: Grammar | undefined;', langium_1.NL, 'export const ', grammar.name, 'Grammar = (): Grammar => loaded', grammar.name, 'Grammar ?? (loaded', grammar.name, 'Grammar = loadGrammarFromJson(`', json, '`));', langium_1.NL);
