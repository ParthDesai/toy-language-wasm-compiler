...
function getTime() {
    const date = new Date();
    return `[${chalk_1.default.gray(`${padZeroes(date.getHours())}:${padZeroes(date.getMinutes())}:${padZeroes(date.getSeconds())}`)}] `;
}
...
function collectKeywords(grammar) {
    ...
    const reachableRules = (0, langium_1.getAllReachableRules)(grammar, false);
    for (const keyword of (0, langium_1.stream)(reachableRules)
        .filter(langium_1.GrammarAST.isParserRule)
        .flatMap(rule => (0, langium_1.streamAllContents)(rule).filter(langium_1.GrammarAST.isKeyword))) {
